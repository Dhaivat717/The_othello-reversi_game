...
def full_minimax(state, player):
    ...
    while(1):
        if player == 'B':
            ...
            # move_sequence_ab.append(('B',r,c))
        else:
            val, r, c = minimax(state, 'W')
            # move_sequence_ab.append(('W',r,c))
        if len(getPossibleMoves(state, player)) == 0:
            if is_terminal_state(state):
                move_sequence.append((player, r, c))
                break
            else:
                if player == 'B':
                    player = 'W'
...
def full_minimax_ab(state, player):
    ...
    while(1):
        ...
        if len(getPossibleMoves(state, player)) == 0:
            if is_terminal_state(state):
                move_sequenceab.append((player, r, c))
                break
            else:
                if player == 'B':
                    player = 'W'
